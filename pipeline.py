...
Tracks window coordinates and sequences.
"""
...
def score_candidates(df_queries, seqA_dict, seqB_dict, log_callback=None, flank=0):
    """
    Score query windows against full target sequences.
    Returns dataframe with nucleotide positions and score.
    """
    rows = []
    log(f"Scoring {len(df_queries)} query windows against {len(seqB_dict)} targets...", log_callback)
    ...
    log(f"Completed scoring {len(df_queries)} windows, total hits: {len(scored_df)}", log_callback)
    return scored_df
